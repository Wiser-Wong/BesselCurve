...
public class StartImageView extends AppCompatImageView implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    public void startAnim(ControlPoint startPoint, ControlPoint endPoint) {
        if (isRunning) return;
        isRunning = true;
        float pointX = (startPoint.x + endPoint.x) / 2;
        float pointY = startPoint.y - PointUtil.dip2px(getContext(), 40);
        ControlPoint controlPoint = new ControlPoint(pointX, pointY);
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ControlPoint point = (ControlPoint) animation.getAnimatedValue();
        setX(point.x);
        setY(point.y);
        invalidate();
    }
